...
#print "start $region_start, end $region_end\n";
...
sub filterExoneratedContigs {
    ...
    foreach my $contig (@contigs) {
        ...
        if ($region_end > $region_start) {
           $overlap_ratio = $overlap / ($region_end - $region_start);
        }
        else {
           $overlap_ratio = 0;
           warn "[WARNING bestcontig_distrib $lib] exon start and end equal in $exonerated_contigs, ratio set to zero\n";
        }
