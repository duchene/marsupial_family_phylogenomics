...
my $cov = "$assemdir/$lib/${lib}_gatkSNPcalls/$lib.ReadGrouped..ReadGrouped.DepthOfCoverageTable";
#my $vcf = "test.vcf";
...
my $Nsites = ();
open(COV, "<$cov") or die "[ERROR vcf2haplofasta $lib] Could not open .vcf input file $vcf\n";
while (my $covline = <COV>)
{
    if ($covline !~ /Total_Depth/) # skip header
	{
	    my ($namepos, $totdepth, undef, undef) = split(/\t/, $covline);
	    if ($totdepth < 16)
	    { 
			my ($locname, $basepos) = split(/:/,$namepos);
			push @{ $Nsites{$locname} }, $basepos;
		}
	}
}
